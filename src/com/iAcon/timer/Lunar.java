 ...
 * <p>
 * <br>
 * Description: 关于农历、节假日等
 * <br>
 * File Name: Lunar.java
 * <br>
 * Copyright: Copyright (C) 2010 All Rights Reserved.
 * <br>
 * <br>
 * @author Lind E-mail:05129098@163.com
 * @create time：May 6, 2010 10:12:23 AM
 * @version: v1.5
 * 
 *       Date              Author      Version     Description
 * ------------------------------------------------------------------
 * 2010-6-13 上午11:04:43 |Lind    　　|v2.5        |Create
 * 
 */
public class Lunar {
	 ...
	 private final static int[] lunarInfo =
	    ...
	    public final static String[] weekName =
	    ...
	    { "0101*元旦", "0214 情人节", "0308 妇女节", "0312 植树节", "0315 消费者权益日", "0401 愚人节", "0501*劳动节",
		    "0504 青年节", "0601 儿童节", "0701 建党节 香港回归纪念", "0801 建军节", "0910 教师节", "1001*国庆节", 
		    "1111 光棍节", "1220 澳门回归纪念", "1224 平安夜", "1225 圣诞节"};

	    /**
	     * 农历节日 *表示放假日
	     */
	    private final static String[] lFtv =
	    { "0101*春节", "0115 元宵节", "0505 端午节", "0707 中国情人节", "0815 中秋节", "0909 重阳节",
		  "1208 腊八节", "1224 小年", "0100*除夕" };

	    /**
	     * 某月的第几个星期几	星期日为:1, 星期六为:7
	     */
	    private static String[] wFtv =
	    { "0521 母亲节", "0631 父亲节", "1145 感恩节"};
	    ...
	    private final static Pattern sFreg = Pattern.compile("^(\\d{2})(\\d{2})([\\s\\*])(.+)$");	//节假日验证（固定日期）

	    private final static Pattern wFreg = Pattern.compile("^(\\d{2})(\\d)(\\d)([\\s\\*])(.+)$");	//节假日验证（月周n）
		...
		Matcher m;
		for (int i = 0; i < Lunar.sFtv.length; i++) {	//sFtv公历节假日
		    m = Lunar.sFreg.matcher(Lunar.sFtv[i]);
		    if (m.find()) {	//驗證是否是節假日
		    	if (sM == Lunar.toInt(m.group(1)) && sD == Lunar.toInt(m.group(2))) {//是節假日
		...
		for (int i = 0; i < Lunar.lFtv.length; i++)	{	//农历节假日
		...
		// 月周节日
		int w, d;
		for (int i = 0; i < Lunar.wFtv.length; i++)	{	//非固定日假期(如：5月的第一个星期的星期一)
	    ...
	    private boolean isSFestival = false;	//是否是公历節假日

	    private boolean isLFestival = false;	//是否是农历节假日

	    private String sFestivalName = "";		//公历節假日名稱

	    private String lFestivalName = "";		//农历节假日名称

	    private String description = "";		//

	    private boolean isHoliday = false;		//放假
...
//	    	System.out.println(l.getTermString());
//	    	System.out.println(Lunar.getSolarTermDay(2010, 3));
////	    	System.out.println(Date.UTC(00, 0, 6, 2, 5, 0));
////	    	System.out.println(Lunar.UTC(1900, 0, 6, 2, 5, 0));
//	    }
		...
		this.solar.setFirstDayOfWeek(Calendar.MONDAY);
		this.solar.setMinimalDaysInFirstWeek(1);
	    ...
	    public String getCyclicalDateString() {
	    	return this.getCyclicaYear() + "年" + this.getCyclicaMonth() 
	    		+ "月" + this.getCyclicaDay() + "日";
	    }
	    ...
	    public String getLunarDateString() {
	    	return this.getLunarYearString() + "(" + this.getAnimalString() + ")" + "年"
				+ this.getLunarMonthString() + "月" + this.getLunarDayString();
	    }
	    ...
	    public String getSolarDateString()  {
	    	return String.valueOf(this.getSolarYear()) + "年" + String.valueOf(this.getSolarMonth())
				+ "月" + String.valueOf(this.getSolarDay()) + "日";
	    }
	    ...
	    public String getWeekName() {
	    	return Lunar.weekName[this.getDayOfWeek() - 1];
	    }
